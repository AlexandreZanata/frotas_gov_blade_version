...
<div class="grid gap-4 md:grid-cols-1">
    ...
    <!-- Senha -->
    <div>
        <x-input-label for="password" value="Senha {{ isset($defaultPassword) && $defaultPassword->exists ? '' : '*' }}" />
        <x-text-input id="password" name="password" type="password" class="mt-1 block w-full" {{ isset($defaultPassword) && $defaultPassword->exists ? '' : 'required' }} />
        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            @if(isset($defaultPassword) && $defaultPassword->exists)
    ...
    <div>
        <x-input-label for="password_confirmation" value="Confirmar Senha {{ isset($defaultPassword) && $defaultPassword->exists ? '' : '*' }}" />
