<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header title="Postos de Combustível" subtitle="Gestão de postos cadastrados" hide-title-mobile
                          icon="fuel"/>
    </x-slot>
    <x-slot name="pageActions">
        <a href="{{ route('gas-stations.create') }}"
            ...
            @forelse($gasStations as $station)
                <tr class="hover:bg-gray-50 dark:hover:bg-navy-700/40">
                    ...
                    <td class="px-4 py-2 text-gray-700 dark:text-navy-200">
                        {{ $station->cnpj ? \App\Http\Controllers\fuel\GasStationController::formatCnpj($station->cnpj) : '—' }}
                    </td>
                    <td class="px-4 py-2">
                    ...
                    <td class="px-4 py-2 whitespace-nowrap text-right">
                        <div class="flex items-center justify-end gap-1">
                            ...
                            <form action="{{ route('gas-stations.destroy', $station) }}" method="POST"
                                  class="inline-block"
